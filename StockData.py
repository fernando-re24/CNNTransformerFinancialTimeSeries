...
class StockData:
    ...
    def df_to_tensor(df: pd.DataFrame, batch_size: int) -> torch.Tensor:
        ...
        data = X.to_numpy().T.astype('float32')  # Enforce float32 at NumPy level
    ...
    def createSequences(df, normalize_cols, seq_len=60, pred_offset=10):
        ...
        all_feature_cols = [col for col in df.columns if col not in ['stock_id']]  # all usable inputs

        for stock_id, group in df.groupby("stock_id"):
            group = group.reset_index(drop=True)
            max_start = len(group) - seq_len - pred_offset + 1

            for start in range(max_start):
                window = group.iloc[start:start+seq_len].copy()

                # Normalize only selected columns within the window
                ...
                # Full input set including normalized and unnormalized
                X_seq = window[all_feature_cols].values
                if X_seq.shape == (seq_len, len(all_feature_cols)):
                    X_list.append(X_seq.T)  # Transpose to (channels, sequence length)

                    # Optional target extraction
                    if 'target' in window.columns:
                        y_list.append(window['target'].iloc[0])
                    else:
                        y_list.append(0)
    ...
    def time_aware_split(X_tensor, y_tensor, train_ratio, val_ratio, test_ratio):
        """
        Time-aware split of time-series data into train/val/test sets.
        Assumes data is already ordered by time.
        """
        N = len(X_tensor)
